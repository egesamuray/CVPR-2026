...
def soft_histogram(x, bins, xmin, xmax, tau=0.1, eps=1e-12):
    if x.dim() > 2:
        x = x.flatten(1)
    B, N = x.shape
    centers = torch.linspace(xmin, xmax, bins, device=x.device)[None, :]
    x = x[:, :, None]
    k = torch.exp(-(x - centers)**2 / (2*(tau**2)))
    k_sum = k.sum(dim=1) + eps
...
class EMABandPrior:
    ...
    def update(self, band, c, p_batch):
        key = (band, int(c)); p_bar = self.get(band, c)
        with torch.no_grad():
...
class SR3Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.optimizer_G = torch.optim.Adam([p for p in self.netG.parameters() if p.requires_grad],
                                            lr=float(train_opt['lr_G']),
                                            weight_decay=wd_G,
                                            betas=(float(train_opt.get('beta1_G', 0.9)), 0.999))
        self.optimizers.append(self.optimizer_G)
        if train_opt.get('lr_scheme') == 'MultiStepLR':
            self.schedulers.append(torch.optim.lr_scheduler.MultiStepLR(self.optimizer_G,
    ...
    def _mmd_loss_band(self, A, B):
        ...
        for i in range(Bsz):
            x = A[i].reshape(-1, 1); y = B[i].reshape(-1, 1)
            if x.size(0) > self.mmd_max_points:
                idx = torch.randperm(x.size(0), device=A.device)[:self.mmd_max_points]; x = x[idx]
            if y.size(0) > self.mmd_max_points:
                idy = torch.randperm(y.size(0), device=A.device)[:self.mmd_max_points]; y = y[idy]
    ...
    def optimize_parameters(self, step):
        ...
        decays = [1.0 / (2**i) for i in range(len(bands_f))]
        ...
        if self.use_ema:
            ...
            G_on  = self.netG.module    if hasattr(self.netG, 'module')    else self.netG
            _ema_update(G_ema, G_on, self.ema_decay)
